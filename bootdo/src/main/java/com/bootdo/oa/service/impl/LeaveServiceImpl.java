...
public class LeaveServiceImpl implements LeaveService {
    ...
    public List<LeaveDO> list(Map<String, Object> map) {
        List<HistoricProcessInstance> historicProcessInstanceList =
                historyService.createHistoricProcessInstanceQuery()
                        .processDefinitionKey(ActivitiConstant.ACTIVITI_PROCESS_LEAVE[0])
        ...
        return historicProcessInstanceList
                .stream().map(e -> {
                            ...
                            leaveDO.setActivitiDO(new ActivitiDO() {{
                                setProcInsId(e.getId());
                                setProcDefId(e.getProcessDefinitionId());
                            }});
                            return leaveDO;
                        }
    ...
    public int save(LeaveDO leave) {
        ...
        vars.put("day", day);
        vars.put("title", ShiroUtils.getUser().getName() + "发起请假申请");
        runtimeService.startProcessInstanceByKey(ActivitiConstant.ACTIVITI_PROCESS_LEAVE[0], leave.getId(),
                vars);
        return r;
    }
